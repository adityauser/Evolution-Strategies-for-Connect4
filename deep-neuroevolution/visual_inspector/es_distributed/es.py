...
class ActionWraps:
    def __init__(self, val, env):
        ...
        self.env = env
    def sample(self):
        return random.sample(self.env.get_state.legal_actions(), 1)[0]
        ...
        if rs.rand() < config.eval_prob:
            ...
            while not noise_inds or time.time() - task_tstart < min_task_runtime:
                ...
                try: 
                    ...
                    #raise TypeError("How the hell it worked", len(task_data.params), policy.num_params, len(v))
                except:
                    policy.set_trainable_flat(task_data.params)
                    print("lens", len(task_data.params), policy.num_params, len(v))
                    raise TypeError("lens", len(task_data.params), policy.num_params, len(v))
