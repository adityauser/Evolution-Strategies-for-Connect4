...
def main(env_id, policy_file, record, stochastic, extra_kwargs):
    ...
    from es_distributed.policies import MujocoPolicy, ESAtariPolicy, ESBoardGame
    from es_distributed.atari_wrappers import ScaledFloatFrame, wrap_deepmind
    from es_distributed.es import get_ref_batch, OpenSpielEnv
    import numpy as np

    is_atari_policy = "NoFrameskip" in env_id
    is_spiel_policy = "four" in env_id

    if not is_spiel_policy:
        env = gym.make(env_id)
    else: 
        env = OpenSpielEnv(env_id)
        record = False
    if is_atari_policy:
    ...
    with tf.Session():
        ...
        elif is_spiel_policy:
            pi = ESBoardGame.Load(policy_file, extra_kwargs=extra_kwargs)
            pi.set_ref_batch(get_ref_batch(env, batch_size=128))
        else:
        ...
        while True:
            if is_atari_policy or is_spiel_policy:
